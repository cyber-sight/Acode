...
### 9.1 Configure the Apple Team ID

The Acode Cordova project does not hardcode `DEVELOPMENT_TEAM` in
`config.xml`, `package.json`, or the canonical iOS plugin source. Supply the
Team ID as a platform build option. This keeps the checkout usable by other
developers and avoids committing personal signing settings.

The Team ID is the short identifier for the Apple Developer team, normally ten
characters. It is different from the Apple ID email address, the certificate
name, and the Acode bundle identifier (`com.foxdebug.acodeios`). Find it in
Xcode under **Signing & Capabilities**, or in the Apple Developer account.

For a device build, export it in the current shell and pass it after Cordova's
`--` separator:
...
To archive and run in one step:
...
`--automaticProvisioning` asks Xcode/Cordova to select or create the matching
development provisioning profile. The Apple account must be added to Xcode,
the iPhone must be trusted and registered to the team, and Developer Mode must
be enabled on the phone. A free or personal team may have Apple-imposed limits
on provisioning duration and capabilities. A paid team is required for normal
development provisioning and capabilities that Apple does not grant to a free
profile.

The upstream `third_party/ish-arm64/iSH.xcodeproj` contains inherited signing
values from the iSH project. Those values are not the Acode app's signing
configuration: `scripts/build-ish.sh` builds static archives with
`CODE_SIGNING_ALLOWED=NO`, and the final signing decision is made by the
generated Cordova Acode project. Do not edit the upstream team values to sign
the Acode app.

If you open the generated project manually, use the **App** target in
`platforms/ios/App.xcworkspace` and select your team under **Signing &
Capabilities**. This is useful for diagnosing provisioning errors, but the
generated `platforms/ios/` project can be regenerated by `cordova prepare`, so
the command-line Team ID is the repeatable configuration.
...
### 9.2 Apple lifecycle restrictions and terminal disconnects

The iOS terminal is an in-process guest runtime. It is not a daemon, service,
or unrestricted Linux VM that iOS must keep alive. The terminal uses a native
PTY and a loopback WebSocket, while the JavaScript WebView owns the visible
terminal. These components have different lifecycle boundaries:

```text
foreground app        -> guest PTY and WebSocket normally run
WebView/socket closes  -> native session may remain and reconnect can recover it
app enters background  -> iOS may suspend the WebView or guest work
background allowance   -> may extend execution temporarily, never indefinitely
app is terminated      -> PTY, guest processes, and reconnect state are lost
explicit stop          -> native session is destroyed and cannot reconnect
```

The runtime requests the available iOS background mechanisms and falls back to
a finite background assertion when a continued-processing task is unavailable
or rejected. These APIs are requests to the operating system, not a promise of
continuous execution. iOS may delay, expire, suspend, or terminate the app due
to lifecycle policy, memory pressure, thermal state, battery policy, or a user
force-quit. A force-quit should always be treated as permanent session loss.

When the app returns to the foreground, the terminal attempts to reconnect by
session UUID if the app process and guest session still exist. Reconnect cannot
restore a session after the process was killed or the guest task exited. A
disconnect indicator therefore means one of two things: a temporary WebView or
WebSocket interruption that may recover, or a native session that no longer
exists. Check the native logs and session state before assuming the rootfs or
shell is corrupt.

For reliable workflows:

- Keep Acode in the foreground during builds, package installs, and migrations.
- Save progress and important output inside the guest filesystem before leaving
  the app.
- Prefer restartable or checkpointed commands over one large uninterruptible
  command.
- Use a remote server, CI runner, or another persistent host for unattended jobs.
- Do not interpret a successful WebSocket reconnect as proof that iOS guarantees
  the guest will continue running in the background.

To diagnose a real device crash or termination, first capture the device logs
through CoreDevice. Legacy `idevicecrashreport` and `idevicesyslog` may not see a
device connected through CoreDevice Wi-Fi transport:

```sh
mkdir -p /tmp/acode-ios-sysdiagnose
xcrun devicectl device sysdiagnose \
  --device "$DEVICE_UDID" \
  --destination /tmp/acode-ios-sysdiagnose \
  --gather-full-logs \
  --timeout 300
```

Extract Acode crash reports from the resulting archive and search for
`IshBackgroundRuntime`, `BGTaskScheduler`, `EXC_CRASH`, `SIGABRT`, or
`termination`. A scheduler rejection should be distinguished from a guest
rootfs failure, a PTY failure, and a WebSocket-only disconnect.
